...
export async function ingestRoutes(app: FastifyInstance) {
  app.post("/refresh", async (req: AuthedRequest) => {
    ...
    // Ingest in the BACKGROUND and respond immediately. A full sync (rate-limited
    // game APIs + occasionally slow/hanging community endpoints) can take well
    // over a minute — longer than the proxy's ~55s limit — which would surface
    // to the user as a misleading "server is waking up" error. The dashboard
    // reloads shortly after to pick up whatever was ingested.
    void ingestUserAll(user.id).catch((e) => console.error("[Ingest] background refresh error:", e));
  ...
  app.post("/refresh/account", async (req: AuthedRequest) => {
    ...
    void ingestGameAccount(account.id).catch((e) => console.error("[Ingest] background account refresh error:", e));
    return { ok: true, started: true };
  });
